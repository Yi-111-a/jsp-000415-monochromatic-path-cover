...
/-!
# GLScratchE3 — Gyárfás–Lehel Thm 3: claims D (both parities) and E

Goal: `claimE_alt` — internal red-completeness of the red branch `S₁`.
The `seg`/nodup/perm machinery and `claimD_even` are copied (privately)
from `GLScratchInt2`.  On top of that we add `e3_claimD_odd` (the mirrored
skip-3 edge) and the claim-E rotations.
-/
...
/-- Claim D, odd first index (mirror of `claimD_even`).  If `a_t–a_{t+3}`
...
private theorem claimD_odd (hXY : Disjoint X Y) (hB' : c3.ClaimB')
  ...
  set x0 := c3.A[t]'(by omega) with hx0_def
  have hmY : m ∈ Y := (c3.a_side hXY _ (by omega)).1.mpr (by omega)
  have hy1Y : y1 ∈ Y := (c3.a_side hXY _ (by omega)).1.mpr (by omega)
  have hy2Y : y2 ∈ Y := (c3.a_side hXY _ ht).1.mpr (by omega)
  have hx0X : x0 ∈ X := (c3.a_side hXY _ (by omega)).2.mpr (by omega)
  have hmU : m ∈ c3.S1U := c3.a_mem_S1U hXY (by omega) (by omega)
  have hy1U : y1 ∈ c3.S1U := c3.a_mem_S1U hXY (by omega) (by omega)
  have hy2U : y2 ∈ c3.S1U := c3.a_mem_S1U hXY ht (by omega)
  have hx0L : x0 ∈ c3.SL := c3.a_mem_SL hXY (by omega) (by omega)
  -- the blue chain edges
  have emx : Color.adjXY G X Y .blue m c3.ux :=
    ⟨Or.inr ⟨hmY, c3.ux_X⟩, fun h ↦ hC c3.ux huL m hmU h.symm⟩
  have exy1 : Color.adjXY G X Y .blue c3.ux y1 :=
    ⟨Or.inl ⟨c3.ux_X, hy1Y⟩, hC c3.ux huL y1 hy1U⟩
  have ey1u2 : Color.adjXY G X Y .blue y1 u₂ :=
    ⟨Or.inr ⟨hy1Y, hu₂X⟩, fun h ↦ hC u₂ hu₂L y1 hy1U h.symm⟩
  have eu2y2 : Color.adjXY G X Y .blue u₂ y2 :=
    ⟨Or.inl ⟨hu₂X, hy2Y⟩, hC u₂ hu₂L y2 hy2U⟩
  have ey2x0 : Color.adjXY G X Y .blue y2 x0 :=
    ⟨Or.inr ⟨hy2Y, hx0X⟩, fun h ↦ hbad h.symm⟩
  have ex0b : ∀ y ∈ c3.B.head?, Color.adjXY G X Y .blue x0 y := by
    intro y hy
    rw [List.head?_eq_some_head c3.hB, Option.mem_some_iff] at hy
    subst hy
    exact ⟨Or.inl ⟨hx0X, B_head_mem_Y hXY c3⟩,
      hB' x0 hx0L (List.getElem_mem _)⟩
  -- red chain `drop (t+4) ++ z :: take t`
  have hR : (c3.A.drop (t + 4) ++ c3.z :: c3.A.take t).IsChain
      (Color.adjXY G X Y .red) := by
    refine chain_app (isChain_drop_of _ (chainA c3)) (chain_cons ?_ ?_) ?_
    · intro y hy
      rw [head?_take (by omega) (by omega), List.head?_eq_some_head c3.hA,
        Option.mem_some_iff] at hy
      subst hy
      exact za1red c3 hXY
    · exact isChain_take_of _ (chainA c3)
    · intro x hx y hy
      rw [List.head?_cons, Option.mem_some_iff] at hy
      subst hy
  ...
  have hR' : ((c3.A.drop (t + 4) ++ c3.z :: c3.A.take (t - 1)) ++ [m]).IsChain
    ...
    rw [e', e]; exact hR
  -- blue chain `m :: ux :: y1 :: u₂ :: y2 :: x0 :: B`
  have hBl : (m :: c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B).IsChain
      (Color.adjXY G X Y .blue) :=
    chain_cons (head_edge emx) (chain_cons (head_edge exy1)
      (chain_cons (head_edge ey1u2) (chain_cons (head_edge eu2y2)
        (chain_cons (head_edge ey2x0) (chain_cons ex0b (chainB c3))))))
  -- nodup via permutation to normal form
  have hperm : List.Perm
      ((c3.A.drop (t + 4) ++ c3.z :: c3.A.take (t - 1)) ++
        m :: (c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B))
      (u₂ :: c3.ux ::
        (c3.A.take (t + 2) ++ c3.A.drop (t + 3) ++ c3.z :: c3.B)) := by
    have hL : (c3.A.drop (t + 4) ++ c3.z :: c3.A.take (t - 1)) ++
          m :: (c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B)
        = c3.A.drop (t + 4) ++ c3.z ::
          (c3.A.take (t - 1) ++
            (m :: c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B)) := by
      simp only [List.append_assoc, List.cons_append, List.nil_append]
    rw [hL]
    calc c3.A.drop (t + 4) ++ c3.z ::
            (c3.A.take (t - 1) ++
              (m :: c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B))
        ~ c3.z :: (c3.A.drop (t + 4) ++
            (c3.A.take (t - 1) ++
              (m :: c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B))) :=
          List.perm_middle
      _ ~ c3.z :: ((c3.A.take (t - 1) ++
              (m :: c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B)) ++
            c3.A.drop (t + 4)) :=
          List.perm_append_comm.cons _
      _ ~ c3.z :: (c3.ux :: u₂ ::
            (c3.A.take (t - 1) ++
              (m :: y1 :: y2 :: x0 :: (c3.B ++ c3.A.drop (t + 4))))) := by
          apply List.Perm.cons
          calc (c3.A.take (t - 1) ++
                  (m :: c3.ux :: y1 :: u₂ :: y2 :: x0 :: c3.B)) ++
                c3.A.drop (t + 4)
              = c3.A.take (t - 1) ++
                (m :: c3.ux :: y1 :: u₂ :: y2 :: x0 ::
                  (c3.B ++ c3.A.drop (t + 4))) := by
                simp only [List.append_assoc, List.cons_append]
            _ ~ c3.A.take (t - 1) ++
                (c3.ux :: m :: y1 :: u₂ :: y2 :: x0 ::
                  (c3.B ++ c3.A.drop (t + 4))) :=
                (List.Perm.swap _ _ _).append_left _
            _ ~ c3.ux :: (c3.A.take (t - 1) ++
                (m :: y1 :: u₂ :: y2 :: x0 :: (c3.B ++ c3.A.drop (t + 4)))) :=
                List.perm_middle
            _ ~ c3.ux :: (c3.A.take (t - 1) ++
      ...
      _ ~ u₂ :: c3.ux ::
            (c3.A.take (t + 2) ++ c3.A.drop (t + 3) ++ c3.z :: c3.B) := by
          calc c3.z :: c3.ux :: u₂ ::
                ((c3.A.take (t + 2) ++ c3.A.drop (t + 3)) ++ c3.B)
              ~ u₂ :: c3.ux :: c3.z ::
                ((c3.A.take (t + 2) ++ c3.A.drop (t + 3)) ++ c3.B) := by
                calc c3.z :: c3.ux :: u₂ ::
                        ((c3.A.take (t + 2) ++ c3.A.drop (t + 3)) ++ c3.B)
                  ...
                  _ ~ u₂ :: c3.ux :: c3.z ::
                      ...
                      (List.Perm.swap _ _ _)
            _ ~ u₂ :: c3.ux ::
                ((c3.A.take (t + 2) ++ c3.A.drop (t + 3)) ++ c3.z :: c3.B) :=
                (List.Perm.cons _ (List.Perm.cons _ List.perm_middle.symm))
  have hndNF : (u₂ :: c3.ux ::
      (c3.A.take (t + 2) ++ c3.A.drop (t + 3) ++ c3.z :: c3.B)).Nodup := by
    have hM := nodup_NF_A c3 (t + 2) (t + 3) (by omega)
    refine List.nodup_cons.mpr ⟨?_, List.nodup_cons.mpr ⟨?_, hM⟩⟩
    · intro h
      rcases List.mem_cons.mp h with e | hM'
      · exact hu₂ne e
